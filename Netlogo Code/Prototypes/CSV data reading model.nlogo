...
to setup
  ...
  ;;Setup Variables
  let nation_number 0
  let resource_number 0

  let result Nationsim.get_nation_variables nation_number resource_number

  set nation_number item 0 result
  set resource_count item 1 result
...
to go
  ...
  [
    ...
    [
      ...
      tick
      ;;ask nations [
      ;;nations.print_data resource_count
      ;;]
    ...
    [
      ...
      show( test_counter )
      set runs runs + 1
      show (word "Going into run " (runs + 1))
    ]
  ]
  [
    clear
...
to-report nationsim.get_nation_variables [number_of_nations number_of_resources]

  file-open "../csvfiles/test-data.txt"

  set number_of_nations nationsim.read
  set number_of_resources nationsim.read

  let i 0
  set resource_names []
  while [i < number_of_resources]
  [
    set resource_names lput nationsim.read resource_names
    set i i + 1
  ]
  ...
  [
    create-nations 1 [
      nations.setup number_of_resources
    ]
    set i i + 1
  ]
  set test_01 nationsim.read
  set test_counter nationsim.read

  Nationsim.get_commands
  file-close

  report list number_of_nations number_of_resources
end
...
to nations.setup [number_of_resources]
 ...
 set wealth_over_time nationsim.read
  ...
  let i 0
  foreach desires [ x ->
    set resource_values lput (x * item i technology_levels) resource_values
    set i i + 1
  ]

end

to nations.print_data [number_of_resources]


  show (word self ": " name)
  show (word "Current Wealth: " last wealth_over_time)
  show (word "Last Population: " last population_over_time)
  let outstring "Current neighbours: "
  ask neighbours
  [
    set outstring (word outstring " " name)
  ]
  show outstring
  ...
  while [i < number_of_resources]
  [
   show  item i resource_names
   show (word "Technology level: " item i technology_levels)
   show (word  "Desire: " item i desires)
   show (word  "Value: " item i resource_values)
   show (word "Last Quantity owned: " item i last resources_over_time )
    set i i + 1
  ]
    ...
    [
      ...
      [
        ...
        [
          ...
          [
            report run-result item 1 values
          ]
          [
            report item 2 values
          ]
        ]
        [
          ifelse (f = "z")
...
to-report nationsim.string_replace [string target new_value]
  ...
  let string_size (length target - 1)
...
to-report run_reporter [standard]
  report runs * standard
end
@#$#@#$#@
GRAPHICS-WINDOW
210
